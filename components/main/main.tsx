...
const Main = () => {
  ...
  return (
    <div
      className={`grid  grid-cols-[auto,auto,1fr,auto] h-screen overflow-hidden ${
        toggleright ? " " : ""
      }`}
    >
      <div
        className="col-span-1 row-span-4 bg-bgside h-screen overflow-hidden"
        onClick={handleClick}
      >
        {toggle ? <Navmobile /> : ""}
      </div>
      ...
      <div className="col-span-2 row-span-4 px-6 flex justify-start flex-col  overflow-clip">
        <InputMain />
      </div>
    </div>
  );
